        ...
        button4.setOnClickListener{
            thread {
                ...
                if (
                    ...
                    val api = URL("https://api.pokemontcg.io/v2/cards?q=set.id:${setsEX.random()} subtypes:EX ($grass$fire$water$lightning$fighting$psychic$colorless$darkness$metal$dragon$fairy)".removeSuffix(" or )") + ")").readText()
                    val json = JSONObject(api)
                    val index = json.getJSONArray("data").length()
                    val data = json.getJSONArray("data").getJSONObject((0 until index).random())
                    largeImage = data.getJSONObject("images").getString("large")
